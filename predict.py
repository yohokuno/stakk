...
class Predictor:
    ...
    def predict(self, input):
        ...
        for yomi, distance, lid, rid, cost, word in results:
            ...
            rank = total - int(1000 * log(1 + len(yomi) - len(unicode(input,'utf-8'))))
...
if __name__ == '__main__':
    #parse options
    parser = OptionParser()
    parser.add_option("-b", dest="base", default="http://localhost:54633/predict/")
    parser.add_option("-c", dest="connection", default="data/mozc-connection.txt")
    (options, args) = parser.parse_args()
    predictor = Predictor(options.base, options.connection)
